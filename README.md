...
> **데이터 확보 현황 (중요)**
> 충북 반도체 기업 대상 개별 데이터 제공 요청에는 회신이 없었습니다. **신규 기업 데이터 없이**
> 현재 보유 데이터와 **공개 데이터**만으로 프로젝트를 완결했습니다. 본 저장소는 외부 데이터
> 유입 없이 그대로 학습·실행·검증이 가능하며, 아래 모든 수치는 동봉된 세 데이터 파일에서
> 계산됩니다.

> **정량 주장은 하나뿐입니다**
> 수율·생산성 개선 수치는 근거가 없어 주장하지 않습니다. 측정으로 뒷받침되는 주장은
> **"같은 정확도의 공정 모델을 60% 적은 TCAD 시뮬레이션으로 얻는다"** (400 run으로 두 target
> 모두 R² ≥ 0.999, [14.1절](#141-충분성--학습곡선)) 하나입니다.
...
| **DOE 설계 비용** | 관행적 full factorial | **학습곡선으로 필요 run 수 측정 → 60% 절감** |
| **물리 일관성** | 검증 안 함 | **10개 (target, 축) 단조성 검사 + dose 축 등장성 투영** |
...
### 3-3. `data/chungbuk_semiconductor_firms_20250624.csv` — 지역 산업 구조 레퍼런스

**충청북도 반도체 기업 현황 공개 데이터(2025-06-24 스냅샷)입니다. 모델 학습에 일절 사용하지 않습니다.**
적용 대상 산업 구조를 주장이 아닌 집계로 서술하기 위한 근거 자료이며,
모든 수치는 `core/chungbuk.py`가 CSV에서 직접 계산합니다.

| 항목 | 값 |
|---|---|
| 등록 기업 수 | **588개** / 6컬럼 (기업명·주소·주요제품·기업분류·업종명) |
| 청주 소재 | **391개 (66.5%)** |
| 반도체 직결 업종 | **86개** |
| └ 장비·부품 제조 | **61개 (직결 업종의 70.9%)** |
| └ 소자·집적회로 | 21개 (24.4%) |
| 상장사(IPO) | **3개 (0.51%)** |
| 소프트웨어 개발·공급 | 41개 |

자세한 해석은 [16. 지역 적합성](#16-지역-적합성--충북-반도체-산업-구조와의-정합)에 있습니다.
...
| **08 DOE 감사 · Physics Guard** | 학습곡선(필요 run 수)·외삽 오차·열예산 축, **단조성 매트릭스**, dose 등장성 투영 전/후 위반, **충북 산업 구조 집계** |
...
├── audit_doe.py                  DOE 정보량 감사 진입점 (충분성·외삽·열예산)
...
│   ├── doe_audit.py              **학습곡선 · 단조성 · 외삽 · 열예산 축 감사**
│   ├── physics_guard.py          **dose 축 등장성 투영 (아티팩트 학습 차단)**
│   ├── chungbuk.py               지역 산업 구조 집계 (모델 학습 미사용)
...
│   └── chungbuk_semiconductor_firms_*.csv      지역 산업 구조 레퍼런스 (공개 데이터)
├── models/
│   ├── surrogate_bundle.joblib           배포 모델 3개 + 전체 리포트
│   └── physics_guard_lattice.npz         단조 투영 격자 캐시
├── outputs/
│   ├── training_report.json          모든 학습 지표의 단일 출처
│   ├── variation_reference.json      실측 산포 측정 결과
│   ├── doe_audit_report.json         DOE 정보량 감사 결과
│   ├── physics_guard_report.json     단조 제약 효과·비용
│   ├── chungbuk_regional_report.json 지역 산업 구조 집계
│   ├── leaderboard_*.csv             target별 모델 비교표
│   └── train_log.txt
├── static/  index.html · css/app.css · js×10 · vendor/chart.umd.js(오프라인 대비)
├── docs/screenshots/             탭별 스크린샷
└── tests/test_pipeline.py        42개 검증 테스트
```
...
`python -m pytest tests -q` → **42/42 PASS**
...
* **크게 깨지는 축이 dose뿐이라는 사실 고정** — 위반율 5% 초과는 `dose → Xj` 2쌍이 유일
* **dose 아티팩트가 확률적 노이즈가 아님** (에너지 간 형태 재현 상관 > 0.85)
* **Physics Guard가 격자 dose 위반을 100% 제거**, ΔXj 항등식 유지, 보정 후에도 단조 보장
* **제약 비용이 모델 자체 group-CV 오차의 절반 미만**임을 강제
* **지역 집계가 원본 레지스터와 일치** (시군 합계 = 전체, 세그먼트 합계 = 직결 업종 수)
* **지역 분석이 수요를 추정하지 않음**을 문구 수준에서 검사

브라우저 실측: 8개 탭 전체 렌더링, **콘솔 에러 0건**.
...
## 14. DOE 정보량 감사 — 1,000 run은 몇 개가 실제로 필요한가

`python audit_doe.py` → `outputs/doe_audit_report.json`. 모든 수치는 실행 시점 측정값입니다.

### 14.1 충분성 — 학습곡선

1,000점 격자에서 N개만 무작위 추출해 학습하고 **나머지 전부**로 평가했습니다
(GP Matérn 5/2, 3회 반복 평균, 판정 기준 R² ≥ 0.999).

| N (학습) | Xj Final R² | Xj MAE (um) | Rsh R² | Rsh MAE (Ω/sq) |
|---|---|---|---|---|
| 40 | 0.996082 | 0.00398 | 0.959902 | 6.167 |
| 60 | 0.994302 | 0.00325 | 0.974866 | 4.427 |
| 80 | 0.997415 | 0.00298 | 0.984689 | 2.965 |
| **120** | **0.999012** | 0.00182 | 0.995228 | 1.985 |
| 160 | 0.999311 | 0.00109 | 0.997079 | 1.408 |
| 240 | 0.999622 | 0.00065 | 0.998719 | 0.859 |
| **400** | 0.999555 | 0.00044 | **0.999652** | 0.448 |
| 600 | 0.999954 | 0.00021 | 0.999836 | 0.264 |
| 800 | 0.999985 | 0.00015 | 0.999936 | 0.191 |

* **Xj Final은 120 run**에서 기준을 넘습니다 (88% 절감).
* **Rsh는 400 run**이 필요합니다. 즉 **Rsh가 DOE 크기를 결정하는 구속 조건**입니다.
* 두 target을 모두 만족하려면 **400 run — TCAD 계산량 60% 절감.**

> **곡선이 완전히 단조롭지 않은 이유**
> N=240(0.999622)이 N=400(0.999555)보다 높게 나온 것은 3회 반복 추출의 표본 노이즈입니다.
> 무작위 부분집합이 매번 달라지므로 이 정도의 역전은 정상이며, 지우지 않고 그대로 싣습니다.
> 판정은 개별 점이 아니라 **기준을 처음 안정적으로 넘는 지점**으로 합니다.

> 이 수치가 뜻하는 것: 본 시스템의 정량적 기여는 "수율을 올린다"가 아니라
> **"같은 정확도의 공정 모델을 60% 적은 시뮬레이션으로 얻는다"** 입니다.
> 전자는 근거가 없고, 후자는 위 표가 근거입니다.
> 표의 모든 값은 `outputs/doe_audit_report.json`에 저장되며 `python audit_doe.py`로 재현됩니다.

### 14.2 외삽 — DOE 상자를 벗어나면

각 축의 양 끝 수준을 제외한 **내부 격자 162점만으로 학습**하고, 남은 **외곽 껍질 838점**으로 평가했습니다.

| Target | 외곽 R² | 외곽 MAE | 내부 MAE | 오차 증폭 |
|---|---|---|---|---|
| `xj_final_um` | 0.9377 | 0.01663 um | 0.000115 um | **×145** |
| `rsh_final_ohm_sq` | 0.9656 | 6.633 Ω/sq | 0.0096 Ω/sq | **×690** |

`core/surrogate.py`의 외삽 차단이 보수적인 이유가 이것입니다. 상자 밖 사용은 막는 것이 맞습니다.

### 14.3 열예산 축 — anneal 2변수는 사실상 1자유도

`(dose, energy)` 조건 내부에서 `log10(Rsh)`를 열예산 축 `Dt = t·exp(−Ea/kT)`의 2차식으로 적합:

| 모델 | 파라미터 | 조건 내부 평균 R² |
|---|---|---|
| Dt 1차원 2차식 (Ea = 3.5 eV, B in Si 문헌값) | **3** | 0.9653 |
| Dt 1차원 2차식 (Ea 자유 적합 → 5.5 eV) | **3** | 0.9851 |
| (temp, time) 2차원 완전 2차식 | 6 | 0.9881 |

**파라미터 절반으로 거의 같은 설명력** → 5×5=25개 anneal 조합은 실질적으로 1개 축입니다.
엔지니어 관점의 함의는 "온도와 시간을 따로 튜닝하지 말고 열예산 하나를 맞춰라"입니다.

단, 입력을 `(log dose, energy, log Dt)` 3개로 **대체**하면 group CV는 오히려 떨어집니다
(Rsh 0.9808 → 0.9634, Xj 0.9524 → 0.9134). 따라서 이 결과는 **해석 자산이지 모델 개선이 아니며**,
배포 모델의 입력은 4개를 그대로 유지합니다.

---

## 15. Physics Guard — 시뮬레이터 아티팩트를 학습하지 않기

`core/physics_guard.py` · `outputs/physics_guard_report.json`

### 15.1 문제: 크게 깨지는 축은 dose 하나뿐이다

나머지 3개 입력을 고정하고 한 축씩 이동시켰을 때, 물리적으로 기대되는 방향의 위반 횟수:

| Target | dose | energy | anneal temp | anneal time |
|---|---|---|---|---|
| `xj_implant_um` (↑ 기대) | **225/800 (28.1%)** | 0/875 | — (무관, 검증됨) | — (무관, 검증됨) |
| `xj_final_um` (↑ 기대) | **219/800 (27.4%)** | 0/875 | 0/800 | 0/800 |
| `rsh_final_ohm_sq` (↓ 기대) | 0/800 | 6/875 (0.7%) | 0/800 | 0/800 |

`xj_implant_um`은 anneal 조건과 무관하므로 그 두 축에는 단조성을 주장하지 않습니다.
따라서 **주장한 (target, 축) 쌍은 10개이고, 그중 7개는 위반이 정확히 0**입니다.

깨지는 3개도 성격이 완전히 다릅니다.

* `dose → Xj` **2개는 27~28%가 깨집니다.** 이건 노이즈로 볼 수 있는 수준이 아닙니다.
* `energy → Rsh`는 875 스텝 중 6개(**0.7%**)만 어긋납니다. 수치 해상도 수준의 이탈입니다.

즉 **문제 축은 dose 하나**이며, Physics Guard도 그 축만 건드립니다.

### 15.2 이것이 물리가 아니라 아티팩트인 근거

고dose 사전 비정질화(pre-amorphisation)가 채널링을 억제해 Xj를 되돌리는 것은 **실재하는 물리 효과**입니다.
그러나 그 메커니즘은 dose에 대해 단조이므로 **가장 높은 dose가 가장 얕아야** 합니다. 관측된 형태는 다릅니다.

에너지별 평균 대비 편차(%):

| dose (cm⁻²) | 5.0e14 | 1.0e15 | 1.5e15 | 2.0e15 | 2.5e15 |
|---|---|---|---|---|---|
| 평균 편차 | −3.478% | +0.436% | +1.059% | **−0.273%** | **+2.256%** |
| 에너지 간 표준편차 | 1.237% | 0.353% | 0.242% | 0.578% | 0.959% |

2.0e15에서만 국소적으로 꺼지고 2.5e15가 **가장 깊습니다**. 비정질화 가설과 반대 방향입니다.
게다가 이 형태는 8개 에너지 전부에서 거의 동일하게 재현됩니다(형태 상관 **r = 0.943**, 최소 0.634).
런마다 흩어지는 확률적 노이즈라면 나타날 수 없는 패턴이고, **dose 수준마다 고정된 오프셋** —
즉 시뮬레이션 측 아티팩트입니다. 제약 없는 서로게이트는 이것을 물리로 학습합니다.

### 15.3 해법: dose 축 등장성(isotonic) 투영

학습된 서로게이트를 기준 격자(21×17×9×9 = 28,917점)에서 평가하고,
**dose 축을 따라서만** PAV(pool-adjacent-violators) 투영한 뒤 다중선형 보간으로 서빙합니다.
다른 축은 손대지 않습니다 (데이터가 이미 만족하므로 제약을 걸면 정확도만 잃습니다).

| Target | 격자 위반(제약 전) | 격자 위반(제약 후) | 제약 비용 (DOE MAE) | 모델 자체 group-CV MAE | 비교 |
|---|---|---|---|---|---|
| `xj_implant_um` | 4,779 / 27,540 (17.4%) | **0** | 0.000731 um | 0.00598 um | **8.2× 작음** |
| `xj_final_um` | 7,061 / 27,540 (25.6%) | **0** | 0.000745 um | 0.00546 um | **7.3× 작음** |

**물리 일관성을 100% 확보하는 대가가 모델 자신의 정직한 오차(group CV)보다 7~8배 작습니다.**
target 전체 범위 대비로는 0.22%입니다.

### 15.4 원본 모델은 교체하지 않습니다

`surrogate.predict_raw()`의 동작은 그대로입니다. Physics Guard는
`physics_guard.predict_guarded()`로 **나란히 비교 가능한 두 번째 의견**으로 제공되며,
ΔXj 항등식(`final − implant`)도 보정 후 그대로 유지됩니다.
"제약을 걸었더니 격자 재현 오차가 늘었다"는 사실을 숨기지 않고 위 표에 그대로 싣습니다.

---

## 16. 지역 적합성 — 충북 반도체 산업 구조와의 정합

`core/chungbuk.py` · `outputs/chungbuk_regional_report.json`
(출처: 충청북도 반도체 기업 현황 공개 데이터, 2025-06-24 스냅샷 588개사)
...
|---|---|
| 등록 기업 | 588개 |
| 청주 집중도 | **391개 (66.5%)** — 진천 36 · 음성 22 · 충주 20 |
| 반도체 직결 업종 | 86개 |
| 그중 **장비·부품 제조** | **61개 (70.9%)** |
| 소자·집적회로 | 21개 (24.4%) |
| **상장사(IPO)** | **3개 (0.51%)** |
| 소프트웨어 개발·공급 | 41개 |
...
* 충북의 반도체 강점은 **소자 양산이 아니라 장비·부품·후공정**입니다. 직결 업종의 70.9%가 장비·부품 제조입니다.
* 상장사가 0.51%에 불과합니다. **상용 TCAD 라이선스를 상시 운용하기 어려운 중소기업이 절대다수**라는 의미입니다.
* 청주 한 곳에 66.5%가 모여 있어 단일 거점 실증이 가능한 밀집도입니다.
* 지역 내 SW 개발·공급 기업이 41개 있어 도입 시 외부 SI 의존 없이 지역 파트너로 연결됩니다.

### 16.3 본 시스템의 위치

대상은 **TCAD 라이선스를 상시 운용하기 어려운 장비·부품·후공정 중소기업**입니다.
이미 보유한 DOE 결과만으로 조건 탐색과 산포 견딤 평가를 수행하게 하는 것이 목적이며,
**TCAD를 대체하지 않습니다.**

데모 공정은 이온주입(전공정)이지만, 파이프라인
`DOE 데이터 → 서로게이트 → XAI → 다목적 최적화 → 산포 기반 robustness`
는 공정에 종속되지 않습니다. 그 근거가 이미 리포지토리 안에 있습니다 —
robustness의 산포 기준은 **PECVD SiON 증착 공정의 실측 17,000점**이며,
이온주입 데이터가 아닌 증착 공정 계측이 같은 파이프라인에 그대로 물려 있습니다.

> **주의 (반드시 함께 언급)**
> 위 분석은 공개 기업 등록 정보의 **업종 분류를 집계한 것**입니다.
> 개별 기업의 TCAD 보유 여부·공정 데이터 보유 여부·도입 의향은 확인된 바 없으며,
> **시장 규모나 수요를 추정하지 않았습니다.** 도입 협의나 실증 계약도 존재하지 않습니다.
...
7. **생산성 향상·수율 개선 수치를 주장하지 않습니다.** 근거 데이터가 없습니다.
   ...
   정량 주장은 **TCAD 계산량 절감(14.1절, 측정값)** 하나뿐입니다.
8. **학습 데이터에 시뮬레이터 아티팩트가 존재합니다.** `dose → Xj`가 기대 방향을
   28.1% 위반하며(15.1절), Physics Guard로 보정 가능하지만 **원인은 제거되지 않았습니다.**
   근본 해결은 해당 dose 수준의 TCAD 재실행입니다.
9. **DOE 상자 밖 오차를 측정했고, 큽니다.** 외곽 껍질에서 MAE가 145~690배 증폭됩니다(14.2절).
   외삽 차단은 편의 기능이 아니라 필수 안전장치입니다.
10. **지역 분석은 업종 분류 집계입니다.** 개별 기업의 도입 의향·TCAD 보유 여부는
    확인된 바 없고, 시장 규모를 추정하지 않았습니다(16절).

---

## 18. 확장 경로 (신규 데이터 확보 시)
...
| 3 | (데이터 불필요) | **연속 후보 공간에서의** Active Learning — TCAD를 루프에 넣고 다음 조건을 모델이 제안 |
| 4 | 다중 implant·후속 공정 데이터 | 이온 종·tilt·채널링·Silicide/Contact까지 변수 확장 |
| 5 | MES/EES 연동 | 드리프트 감지 후 재학습 파이프라인 |

> **3단계에 대한 정정 (측정 결과 반영)**
> 초기 버전은 "Active Learning이 현재 데이터만으로 즉시 구현 가능하며 우선 확장 대상"이라고
> 기술했습니다. 실제로 측정한 결과 **그렇지 않습니다.**
> 1,000점 full factorial 격자를 후보 풀로 두고 GP 최대분산 획득함수와 무작위 추출을 비교하면:
>
> | N | Active (max-variance) R² | Random R² |
> |---|---|---|
> | 60 | 0.9757 | 0.9663 |
> | 100 | 0.9936 | **0.9942** |
> | 200 | 0.9985 | 0.9980 |
>
> 이미 균형 잡힌 격자에서는 최대분산 획득함수가 상자 모서리만 선택하므로 **이득이 없습니다.**
> Active Learning이 의미를 가지려면 후보 공간이 **연속**이고 TCAD가 루프 안에 있어야 합니다.
> 시뮬레이션 절감의 근거는 Active Learning이 아니라 **14.1절의 학습곡선**입니다.
